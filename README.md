...
**推荐配置**：在obsidian里安装claudian，使用opus模型，claude当对话窗口，obsidian GUI当阅读器。
## 核心原理:一轮怎么读

一本书入库后(见下「快速开始」),读每一章都是同样的**三阶段循环**:

![echo-reading 一轮阅读流程](docs/principle.png)

1. **备读** — 你说「开始读《X》/ 继续读《X》下一章」,Claude 自动定位进度(上次读到哪)、找到对应原文,把它**备成阅读材料**:原文 + 字词注、白话直译、注家对照、值得想的问题。
2. **深读 · 对话(主线)** — 你读这份材料,把 Claude 当成博学的老师:有不懂、不服、有联想随时抛给它,在一来一回里把每一段读懂、读透。这是全程的主线。
3. **收尾** — 一章读完、进下一章前,自动标记进度。

两件事**全程自动、绝不喧宾夺主**:对话里冒出的理解、连接、共鸣、疑问静默沉淀进 `insight/`,进度随读随记进 `progress.md`。

底层把书分成两层——`books/` 是只读的**原文事实层**,`insight/` 是对话里长出来的**衍生层**(详见「目录结构」)。
...
docs/principle.html      原理图源文件(改字后重渲 principle.png)
```
