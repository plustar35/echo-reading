...
#
#   用法： scripts/render-video.sh ROOT/video/<书>/chNN/NN/分镜.js
#   前置： node(内置 WebSocket,≥21) · Google Chrome · ffmpeg；先跑 gen-tts.js 出音轨+时长。
#   产物： 分镜.js 同目录下的 领读视频.mp4
set -e
ENGINE_DIR="$(cd "$(dirname "$0")" && pwd)"
SB="$1"
[ -z "$SB" ] && { echo "用法: scripts/render-video.sh <分镜.js 路径>"; exit 1; }
SB_ABS="$(cd "$(dirname "$SB")" && pwd)/$(basename "$SB")"
CH_DIR="$(dirname "$SB_ABS")"

FPS=24

# ▸ 0/3 对齐：用 gen-tts 的时间轴回填 durs[] + 强校验(尤其 T0 不得越过卷轴铺好时刻)。
# 非零退出(校验不过 / 对齐失败)则【中止渲染】，绝不渲出尬白片。无时间轴时 align 自跳过。
echo "▸ 0/3 对齐时长 + 校验 (align)…"
if ! node "$ENGINE_DIR/align-durs.js" "$SB_ABS"; then
  echo "✗ align 校验未过，已中止渲染（修好分镜再重渲）" >&2
  exit 1
fi
...
' "$SB_ABS")
ASSETS_DIR="$(cd "$ENGINE_DIR/.." && pwd)/assets/$ASSET_ID"
...
# renderer URL：data 用相对 engine/ 的路径,经 Node 编码中文/特殊字符
URL=$(node -e '
...
echo "▸ 1/3 离屏渲染透明叠层 (~5min)…  data=$(basename "$CH_DIR")/分镜.js  全长 ${AUDIO_DUR}s"
OUT="$FRAMES" FPS=$FPS node "$ENGINE_DIR/render-overlay.js" "$URL"

echo "▸ 2/3 背景层视频 (片头 ${INTRO_VID}s → 背景循环补 ${LOOP_DUR}s)…"
...
echo "▸ 3/3 合成 overlay + 口播音轨 → 领读视频.mp4…"
