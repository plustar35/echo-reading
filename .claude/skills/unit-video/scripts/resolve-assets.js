 ...
 *       没给则看 assets/<书名>/ 下的套目录——恰好一套 → 自动用它；多套 → 抛错（让 agent
 *       先用 AskUserQuestion 问用户、再带 ASSETS 重跑）；没有 → 返回 null（用内置缺省）。 */
"use strict";
const fs = require("fs");
const path = require("path");

const ASSETS_ROOT = path.join(__dirname, "..", "assets");
const isSet = p => fs.existsSync(path.join(p, "底板.png")) || fs.existsSync(path.join(p, "配置.js"));
...
function resolveAssetId(book) {
  ...
  if (sets.length > 1)
    throw new Error(`《${book}》有多个素材套（${sets.join(" / ")}）——先问用户用哪套，再带 ASSETS=${book}/<素材id> 重跑`);
  return null;
}
...
module.exports = { resolveAssetId, loadAssetConfig, ASSETS_ROOT };
