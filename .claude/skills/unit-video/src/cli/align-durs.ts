...
if (!SB_IN) { console.error("用法: node dist/cli/align-durs.js <分镜.js 路径>"); process.exit(1); }
const SB_ABS = path.resolve(SB_IN);
if (!fs.existsSync(SB_ABS)) { console.error("找不到分镜: " + SB_ABS); process.exit(1); }
...
let SB;
try { SB = require(SB_ABS); }
catch (e) { console.error("无法 require 分镜: " + SB_ABS + "\n  " + e.message); process.exit(1); }

let timeline;
try { timeline = JSON.parse(fs.readFileSync(TIMELINE_ABS, "utf8")); }
catch (e) { console.error("无法解析时间轴 JSON: " + TIMELINE_ABS + "\n  " + e.message); process.exit(1); }
...
if (result.isBlockLevel) console.log("· 时间轴为段级降级(granularity=block)：句中切点按整段字比例估时，精度较粗");
if (result.errors.length) fail("分镜校验失败：\n   · " + result.errors.join("\n   · "));
...
function findArrayProp(src, prop) {
  ...
  for (let i = start; i < src.length; i++) {
    ...
    if (ch === "[") depth++;
      ...
      depth--;
      if (depth === 0) return { valueStart: start, valueEnd: i + 1, propEnd: i + 1 };
    }
  }
  return null;
}
...
function replaceArrayProp(src, prop, value) {
  ...
  if (!loc) return { text: src, replaced: false };
...
function writeDurArrays(durs, stepDurs) {
  ...
  if (!r.replaced) {
    ...
    if (!ins.inserted) fail("分镜里找不到 durs，无法插入 stepDurs。");
    src = ins.text;
  } else src = r.text;
