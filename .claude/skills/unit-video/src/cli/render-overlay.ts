...
if(!FILE){console.error('用法: node render-overlay.js "<renderer URL ?...&record=1>"');process.exit(1);}
...
let ws,msgId=0;const pending=new Map();let evWaiters=[];
function connect(url){return new Promise((res,rej)=>{ws=new WebSocket(url);ws.onopen=()=>res();ws.onerror=e=>rej(new Error('ws err'));
  ws.onmessage=ev=>{const m=JSON.parse(ev.data);
    if(m.id&&pending.has(m.id)){const{resolve,reject}=pending.get(m.id);pending.delete(m.id);m.error?reject(new Error(m.method+': '+JSON.stringify(m.error))):resolve(m.result);}
    else if(m.method){evWaiters=evWaiters.filter(w=>{if(w.method===m.method){w.resolve(m.params);return false;}return true;});}};});}
function send(method,params={},sessionId){return new Promise((resolve,reject)=>{const id=++msgId;pending.set(id,{resolve,reject});const msg={id,method,params};if(sessionId)msg.sessionId=sessionId;ws.send(JSON.stringify(msg));});}
...
(async()=>{
  ...
  let wsurl;
  for(let i=0;i<60&&!wsurl;i++){try{const v=await getJSON(`http://127.0.0.1:${PORT}/json/version`);wsurl=v.webSocketDebuggerUrl;}catch(e){await sleep(100);}}
  if(!wsurl)throw new Error('chrome not up');
  await connect(wsurl);
  ...
  for(let i=0;i<100&&!totalMs;i++){
    ...
    if(!totalMs)await sleep(100);
  }
  if(!totalMs)throw new Error('no __RENDER_TOTAL_MS (record mode not active?)');
  let frames=Math.ceil(totalMs/FRAME_MS)+1;
  if(MAXF)frames=Math.min(frames,MAXF);
  ...
  for(let f=0;f<frames;f++){
    ...
    if(VERBOSE)process.stderr.write(`f${f} adv-sent `);
    await withTO(exp,8000,'budgetExpired f'+f);
    if(VERBOSE)process.stderr.write('expired ');
    ...
    if(VERBOSE)process.stderr.write('shot\n');
    fs.writeFileSync(path.join(OUT,String(f).padStart(5,'0')+'.png'),Buffer.from(shot.data,'base64'));
    if(f%200===0||f===frames-1){const el=(Date.now()-t0)/1000;process.stderr.write(`  frame ${f+1}/${frames}  ${el.toFixed(0)}s\n`);}
  }
